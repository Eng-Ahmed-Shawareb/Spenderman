...
/**
 * Class representing ClsTransactionController.
 *
...
public class ClsTransactionController extends ABaseController implements IObserver {
    ...
    @FXML
    private ToggleButton _expenseToggle;

    @FXML
    private ToggleButton _depositToggle;

    @FXML
    private ToggleButton _targetWallet;

    @FXML
    private ToggleButton _targetGoal;

    @FXML
    private ComboBox<String> _targetCombo;

    @FXML
    private ComboBox<String> _categoryCombo;

    @FXML
    private TextArea _noteArea;

    @FXML
    private Label _addErrorLabel;

    @FXML
    private VBox _editFormPanel;

    @FXML
    private TextField _editAmountField;

    @FXML
    private ToggleButton _editExpenseToggle;

    @FXML
    private ToggleButton _editDepositToggle;

    @FXML
    private ToggleButton _editTargetWallet;

    @FXML
    private ToggleButton _editTargetGoal;

    @FXML
    private ComboBox<String> _editTargetCombo;

    @FXML
    private ComboBox<String> _editCategoryCombo;

    @FXML
    private TextArea _editNoteArea;

    @FXML
    private Label _editErrorLabel;

    @FXML
    private VBox _transactionRows;
    ...
    public void initialize() {
        ...
        _targetGroup.selectedToggleProperty().addListener((obs, old, newToggle) -> {
            ...
            if (newToggle == _targetWallet && _wallets != null) {
                ...
                _targetCombo.getItems().addAll(_goals.stream().filter(g -> g.getStatus() == EnGoalState.ACTIVE).map(ClsSavingGoal::get_name).collect(Collectors.toList()));
            }
        });
        ...
        _editTargetGroup.selectedToggleProperty().addListener((obs, old, newToggle) -> {
            ...
            if (newToggle == _editTargetWallet && _wallets != null) {
                ...
                _editTargetCombo.getItems().addAll(_goals.stream().filter(g -> g.getStatus() == EnGoalState.ACTIVE).map(ClsSavingGoal::get_name).collect(Collectors.toList()));
            }
        });
        if ($currentUser != null) {
            refreshData();
    ...
    /**
     * Method to _handleAddTransaction.
     */
    @FXML
    ...
    /**
     * Method to _handleSaveEdit.
     */
    @FXML
    private void _handleSaveEdit() {
        if (_currentEditTx == null)
            return;
    ...
    /**
     * Method to _handleEditTransaction.
     *
     * @param tx the tx
     */
    @FXML
    private void _handleEditTransaction(ClsTransaction tx) {
        ...
        if (tx.get_walletID() > 0 && _wallets != null) {
            ...
            ClsWallet w = _wallets.stream().filter(x -> x.get_walletID() == tx.get_walletID()).findFirst().orElse(null);
            if (w != null)
                _editTargetCombo.setValue(w.get_name());
        } else if (tx.get_savingGoalID() > 0 && _goals != null) {
            _editTargetGoal.setSelected(true);
            ClsSavingGoal g = _goals.stream().filter(x -> x.get_goalID() == tx.get_savingGoalID()).findFirst().orElse(null);
            if (g != null)
                _editTargetCombo.setValue(g.get_name());
        }
        if (_categories != null) {
            ClsCategory c = _categories.stream().filter(x -> x.get_categoryID() == tx.get_categoryID()).findFirst().orElse(null);
            if (c != null)
                _editCategoryCombo.setValue(c.get_name());
        }
    ...
    /**
     * Method to _handleDeleteTransaction.
     *
     * @param tx the tx
     */
    ...
    /**
     * Method to _updateCategoryCombo.
     */
    private void _updateCategoryCombo() {
        ...
        _categoryCombo.getItems().addAll(_categories.stream().filter(c -> c.get_type() == type).map(ClsCategory::get_name).collect(Collectors.toList()));
    ...
    /**
     * Method to _updateEditCategoryCombo.
     */
    private void _updateEditCategoryCombo() {
        ...
        _editCategoryCombo.getItems().addAll(_categories.stream().filter(c -> c.get_type() == type).map(ClsCategory::get_name).collect(Collectors.toList()));
