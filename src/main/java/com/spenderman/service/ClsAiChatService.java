...
/**
 * Class representing ClsAiChatService.
 *
...
public class ClsAiChatService {
    ...
    /**
     * Method to _prepareFinancialContext.
     *
        ...
        try {
            ...
            try {
                ...
                for (ClsTransaction t : transactions) {
                    if (t.get_type() == EnTransactionType.EXPENSE)
                        totalExpenses += t.get_amount();
                    else if (t.get_type() == EnTransactionType.DEPOSIT)
                        totalIncome += t.get_amount();
                }
                context.append(String.format("Total income: %.2f EGP. Total expenses: %.2f EGP. Transactions count: %d. ", totalIncome, totalExpenses, transactions.size()));
            } catch (Exception e) {
            }
            try {
            ...
            try {
                ...
                if (!goals.isEmpty()) {
                    ...
                    for (ClsSavingGoal g : goals) {
                        context.append(String.format("[Target: %.2f, Currently Saved: %.2f, Status: %s] ", g.get_targetAmount(), g.get_currentSaved(), g.getStatus()));
                    }
                } else {
    ...
    /**
     * Method to getAiResponse.
     *
     * @param userMessage the userMessage
     * @param userId the userId
     * @return the CompletableFuture<String>
     */
    public CompletableFuture<String> getAiResponse(String userMessage, int userId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                JSONObject requestBody = new JSONObject().put("model", MODEL).put("messages", new JSONArray().put(new JSONObject().put("role", "system").put("content", "You are a smart financial assistant inside an app called 'Spenderman'. " + "User financial summary: [" + context + "]. " + "Give short and helpful financial advice. " + "Always reply in the same language the user uses.")).put(new JSONObject().put("role", "user").put("content", userMessage))).put("max_tokens", 500);
                HttpRequest request = HttpRequest.newBuilder().uri(URI.create(API_URL)).header("Content-Type", "application/json").header("Authorization", "Bearer " + API_KEY).POST(HttpRequest.BodyPublishers.ofString(requestBody.toString())).build();
