...
/**
 * Class representing ClsDashboardController.
 *
...
public class ClsDashboardController extends ABaseController implements IObserver {
    ...
    private static final String[] PALETTE = { "#F5A623", "#8875F5", "#F472B6", "#4B9EF8", "#22C97A", "#EF4444", "#F59E0B", "#6366F1" };
    ...
    /**
     * Method to _loadWalletBreakdown.
     */
    ...
    private void _loadExpenseChart(List<ClsTransaction> txns, List<ClsCategory> cats) {
        ...
        for (Map.Entry<Integer, Double> e : totals.entrySet()) {
            ...
            String hex = catMap.containsKey(e.getKey()) ? catMap.get(e.getKey()).get_hexColor() : PALETTE[i % PALETTE.length];
    ...
    /**
     * Method to _loadWalletChart.
     */
    private void _loadWalletChart() {
        List<ClsWallet> wallets = _walletService.getByUser($currentUser.getUserID());
        double grandTotal = wallets.stream().mapToDouble(w -> Math.max(0, w.get_balance())).sum();
        ...
        for (int i = 0; i < wallets.size(); i++) {
            ...
            double balance = Math.max(0, w.get_balance());
    ...
    public void update(EnEvenType evenType, Object data) {
        if (evenType == EnEvenType.TRANSACTION_ADDED || evenType == EnEvenType.TRANSACTION_UPDATED || evenType == EnEvenType.TRANSACTION_DELETED || evenType == EnEvenType.WALLET_ADDED || evenType == EnEvenType.WALLET_UPDATED || evenType == EnEvenType.WALLET_DELETED || evenType == EnEvenType.GOAL_ADDED || evenType == EnEvenType.GOAL_UPDATED || evenType == EnEvenType.GOAL_DELETED || evenType == EnEvenType.CYCLE_ADDED || evenType == EnEvenType.CYCLE_UPDATED || evenType == EnEvenType.CYCLE_DELETED || evenType == EnEvenType.CATEGORY_ADDED || evenType == EnEvenType.CATEGORY_UPDATED || evenType == EnEvenType.CATEGORY_DELETED) {
            refreshData();
        }
    }
}
